...
def train_step(
    ...
    # (e.g. dims 14:32 of a 32-D pi-0.5 head on a 14-D bimanual EEF action space) don't pull mean/std toward 0.
...
def main(config: _config.TrainConfig):
    ...
    # Log images from first batch to sanity check (skip for state-only models).
    if batch[0].images:
        images_to_log = [
